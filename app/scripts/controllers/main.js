...
angular.module('tetApp')
  .controller('MainCtrl', ['$scope', '$http', '$location', '$routeParams', '$window', 'dsv', 'cfpLoadingBar', 'index',
                  function ($scope,   $http,   $location,   $routeParams,   $window,   dsv,   cfpLoadingBar,   index) {
    ...
    function loadParams(def) { // TODO: a service?
      ...
      s.c = (angular.isArray(s.c)) ?
        s.c.map(function(d) { return +d; }) : [+s.c];
    ...
    function loadData() {
      $scope.search.head = +$scope.search.head;
    ...
    $scope.next = function() {
      $scope.search.skip = ($scope.search.skip || 0) + (+$scope.search.head);
    };

    $scope.prev = function() {
      $scope.search.skip = Math.max(($scope.search.skip || 0) - +$scope.search.head,0);
    };
